...
// ===== Custom Colorscale: NeonGreen =====
// Build a Plotly colorscale matching the OpenCV NeonGreen LUT
function neonGreenScale() {
  const scale = [];
  for (let i = 0; i < 256; i++) {
    const g = Math.min(255, 2 * i);
    const b = Math.floor(0.3 * g);
    const r = Math.floor(0.1 * g);
    const t = i / 255;
    scale.push([t, `rgb(${r},${g},${b})`]);
  }
  // ensure exact 0 and 1 stops exist
  scale[0] = [0, scale[0][1]];
  scale[scale.length - 1] = [1, scale[scale.length - 1][1]];
  return scale;
}
function getColorScale(name) {
  if (name === "NeonGreen") return neonGreenScale();
  return name; // for built-in Plotly named scales
}
...
// Try to sync initial colormap with <select> default (so NeonGreen works on first draw)
const cmapSelect = el("cmap");
if (cmapSelect && cmapSelect.value) gState.cmap = cmapSelect.value;
...
function showLoader() {
  ...
  if (analyzeBtn) {
    analyzeBtn.dataset._label = analyzeBtn.textContent;
...
/* ================= Cleaner toggles (mutually exclusive) ================= */
const denoiseChk = el("denoiseChk");
const mlCleanChk = el("mlCleanChk"); // may not exist if you haven't added it to HTML yet

if (denoiseChk && mlCleanChk) {
  denoiseChk.addEventListener("change", () => {
    if (denoiseChk.checked) mlCleanChk.checked = false;
  });
  mlCleanChk.addEventListener("change", () => {
    if (mlCleanChk.checked) denoiseChk.checked = false;
  });
}

// Normalize to one of: 'none' | 'specsub' | 'mlmask'
function computeCleanMode() {
  const d = !!(denoiseChk && denoiseChk.checked);
  const m = !!(mlCleanChk && mlCleanChk.checked);
  return m ? "mlmask" : d ? "specsub" : "none";
}

function updateHeaderTagFromResponse(data) {
  const tagEl = document.querySelector(".tag");
  if (!tagEl) return;
  tagEl.textContent =
    data?.clean_mode === "mlmask"
      ? "ml-cleaned"
      : data?.denoised || data?.clean_mode === "specsub"
      ? "denoised"
      : "enhanced";
}
...
el("uploadForm").addEventListener("submit", async (e) => {
  ...
  const clean_mode = computeCleanMode();
  const denoise = clean_mode === "specsub"; // keep legacy flag for server compatibility

  const params =
    method === "raw"
      ? {
          max_formants: parseInt(el("maxFormants").value || 20, 10),
      ...
      : {
          ...
          min_len: parseInt(el("minLen").value || 20, 10),
          denoise,
          clean_mode,
        };
  ...
  try {
    ...
    updateHeaderTagFromResponse(data);
    renderAll();
  } catch (err) {
...
/* ================= Options toggles ================= */
el("cmap").addEventListener("change", (e) => {
  gState.cmap = e.target.value;
  renderSpectrogram();
});
...
if (toggleF0El)
  toggleF0El.addEventListener("change", (e) => {
    gState.showF0 = e.target.checked;
    renderSpectrogram();
  });
if (toggleFormantsEl)
  toggleFormantsEl.addEventListener("change", (e) => {
    gState.showFormants = e.target.checked;
    renderSpectrogram();
  });

const toggleContoursEl = document.getElementById("toggleContours");
if (toggleContoursEl)
  toggleContoursEl.addEventListener("change", (e) => {
    gState.showContours = e.target.checked;
    renderSpectrogram();
  });

/* ================= Downloads ================= */
el("downloadPng").addEventListener("click", async () => {
  Plotly.toImage(el("specPlot"), { format: "png", height: 700, width: 1200 }).then(
    (dataUrl) => {
...
el("dlDist").addEventListener("click", () =>
  downloadCsv(gState?.lastResult?.tables?.distance_csv, "distance_matrix.csv")
);
el("dlTime").addEventListener("click", () =>
  downloadCsv(gState?.lastResult?.tables?.time_csv, "time_ranges.csv")
);
...
function buildTable(containerId, rows, opts = {}) {
  const { headerRow = true, indexCol = true, partitionEvery = 5, numeric = true } =
    opts;
  ...
  const headers = headerRow
    ? rows[0]
    : Array.from({ length: rows[0].length - (indexCol ? 1 : 0) }, (_, i) => `C${i + 1}`);
...
function renderTables() {
  ...
  if (!distRows.length) {
    ...
    buildTable("distTable", distRows, {
      headerRow: true,
      indexCol: true,
      partitionEvery: 5, // change to taste
      numeric: true,
    });
  }
...
function renderBark() {
  ...
  for (let i = 0; i < bark.edges.length - 1; i++) {
    const left = bark.edges[i],
      right = bark.edges[i + 1];
    ...
    [
      {
        ...
        y,
        type: "bar",
        hovertemplate: "%{x}<br>Avg dB: %{y:.2f}<extra></extra>",
      },
    ],
    {
...
function renderSpectrogram() {
  ...
  const heat = {
    ...
    y: y,
    type: "heatmap",
    colorscale: getColorScale(gState.cmap),
    zmin: -80,
    zmax: 0,
  ...
  if (res.mode === "Raw Audio") {
    if (gState.showF0 && res.f0) {
      traces.push({
        ...
        name: "F0",
        line: { width: 1.5 },
        hovertemplate: "F0<br>t=%{x:.3f}s<br>f=%{y:.0f}Hz<extra></extra>",
        yaxis: "y",
      });
    }
    if (gState.showFormants && res.formants) {
      res.formants.forEach((fr, idx) => {
        traces.push({
          ...
          name: `F${idx + 1}`,
          line: { width: 1 },
          hovertemplate: `F${idx + 1}<br>t=%{x:.3f}s<br>f=%{y:.0f}Hz<extra></extra>`,
          yaxis: "y",
        });
      });
    }
    ...
    if (gState.showContours && res.contours) {
      res.contours.forEach((c) => {
        traces.push({
          ...
          name: c.name,
          line: { width: 1 },
          hovertemplate: `${c.name}<br>t=%{x:.3f}s<br>f=%{y:.0f}Hz<extra></extra>`,
          yaxis: "y",
        });
      });
    }
